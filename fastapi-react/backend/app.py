...
async def get_horarios(fuso_horario: str = 'America/Sao_Paulo'):#fuso_horario é um parâmetro de query que será passado na url, poderia usar uma requisição post e passar o fuso horário no corpo da requisição também
    if fuso_horario not in pytz.all_timezones_set: #usando o set para melhorar a performance no lookup #pesquisar sobre lazy sets python
...
@app.get("/alunos", tags=["alunos"])#rota para retornar os alunos e ser consumida pelo react
